...
const Balances = () => {
    ...
    const loadHistory = useCallback(() => {
        ...
        setTimeout(() => {
            setMonths(prev => {
                ...
                // Find oldest transaction date to know when to stop
                const oldestTxn = (appData.transactions || []).reduce((oldest, t) => t.date < oldest ? t.date : oldest, '9999-99-99');
                const oldestYear = parseInt(oldestTxn.split('-')[0]) || new Date().getFullYear() - 1;
                const oldestMonth = (parseInt(oldestTxn.split('-')[1]) || 1) - 1;

                if (d.getFullYear() < oldestYear || (d.getFullYear() === oldestYear && d.getMonth() < oldestMonth)) return prev;
    ...
    }, [isLoading, appData.transactions]);
    ...
    const renderedData = useMemo(() => {
        ...
        return months.map(m => {
            const monthId = `${m.year}-${String(m.month + 1).padStart(2, '0')}`;
